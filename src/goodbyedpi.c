...
static char* dumb_memmem(const char* haystack, UINT hlen, const char* needle, size_t nlen) {
...
static void mix_case(char *pktdata, UINT pktlen) {
    UINT i;
...
static int is_passivedpi_redirect(const char *pktdata, UINT pktlen) {
...
static int find_header_and_get_info(const char *pktdata, UINT pktlen,
                const char *hdrname,
                char **hdrnameaddr,
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "1234prsaf:e:mwk:n", long_options, NULL)) != -1) {
        switch (opt) {
            ...
            case 'e':
                do_fragment_https = 1;
                https_fragment_size = atousi(optarg, "Fragment size should be in range [0 - 0xFFFF]\n");
                break;
            case 'w':
                do_http_allports = 1;
                break;
